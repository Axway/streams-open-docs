...
You must specify a hostname for the ingress installed with the helm Streams. You can change it with the `ingress.host` parameter. Throughout this documentation, we are using _k8s.yourdomain.tld_ as an example value.

If you do not have one yet, you can use any temporary value and edit it later.
For instance, for testing purpose, you should install Streams and use the DNS name generated by your load balancer Cloud provider. You can retrieve it by using the following command:
...
For example, you can specify another value to the _cors allow origin_ configuration with the `ingress.annotations.nginx.ingress.kubernetes.io/cors-allow-origin` parameter. For instance, if you want to allow cross origin request from the domain name _https://origin-site.com_:
