...
Cross-Origin Resource Sharing (CORS) is disabled by default. You can enable it by setting the [Helm parameters](#helm-parameters) `ingress.annotations.nginx.ingress.kubernetes.io/enable-cors` to `"true"`.

You can configure it by adding annotations to the `ingress` parameter (refer to [Nginx documentation](https://kubernetes.github.io/ingress-nginx/user-guide/nginx-configuration/annotations/#enable-cors) for further details):
